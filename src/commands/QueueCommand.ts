...
export class QueueCommand extends BaseCommand {
    ...
    public execute(message: IMessage): any {
        const embed = createEmbed("info")
            .setAuthor("Cola de música", message.client.user?.displayAvatarURL() as string);
        ...
        if (Number(message.guild?.queue?.songs.size) > 12) {
            ...
            let index = 0;
            embed.setDescription(indexes[index]).setFooter(`Página ${index + 1} de ${indexes.length}`, "https://i.imgur.com/aPPapMg.png");
